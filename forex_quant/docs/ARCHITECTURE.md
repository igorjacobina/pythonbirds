# Forex Quant — Arquitetura de Alta Performance
...
- `execution.py`: simulação de preenchimento (market/limit/stop) com a regra
  conservadora de pior preço dentro da vela.
- `portfolio.py`: contabilidade de posição, equity, margem, P&L realizado/aberto.
...
- `metrics.py`: métricas institucionais — CAGR, Sharpe, Sortino, Calmar,
  max drawdown e duração, Profit Factor, expectancy, VaR/CVaR, exposição.
...
### L5 — `research` (próxima fase)
- Mineração de padrões, walk-forward, otimização com penalização de overfitting
  (deflated Sharpe, PBO), modelos (XGBoost → sequência/atenção).
...
- **Fase 0 (esta entrega)**: Core + Data Pipeline + Backtest Engine + Metrics,
  com demo rodável em dados sintéticos e suíte de testes.
- **Fase 1**: Conector MT5 real + ingestão de histórico Dukascopy + storage.
- **Fase 2**: Biblioteca de features e detecção de padrões.
- **Fase 3**: Walk-forward + otimização com controle de overfitting.
