...
class RiskReport:
    """Indicadores de risco/margem do backtest (estilo MT5)."""

    min_margin_level: float        # menor nível de margem enquanto posicionado
    max_used_margin: float         # pico de margem utilizada
    avg_used_margin: float         # margem média enquanto posicionado
    n_rejected_orders: int         # ordens bloqueadas por margem livre insuficiente
    n_stopouts: int                # liquidações forçadas pela corretora
    blew_account: bool             # equity tocou <= 0 em algum momento
    ...
    def summary(self) -> str:
        ml = (
            "—" if self.min_margin_level >= float("inf")
            else f"{self.min_margin_level:.0%}"
        )
        return (
            f"Nível de margem mín. : {ml}\n"
            f"Margem usada (pico)  : {self.max_used_margin:,.2f}\n"
            f"Margem usada (média) : {self.avg_used_margin:,.2f}\n"
            f"Ordens rejeitadas    : {self.n_rejected_orders}\n"
            f"Stop-outs            : {self.n_stopouts}\n"
            f"Conta zerada         : {'SIM' if self.blew_account else 'não'}"
        )
...
def compute_risk(
    ...
    *,
    ...
    """Resume os indicadores de risco a partir das séries do engine."""
    equity = np.asarray(equity, dtype=np.float64)
    margin_level = np.asarray(margin_level, dtype=np.float64)
    used_margin = np.asarray(used_margin, dtype=np.float64)

    in_position = used_margin > 0.0
    levels = margin_level[(margin_level < sentinel) & in_position]
    min_ml = float(levels.min()) if levels.size else float("inf")

    used_active = used_margin[in_position]
    return RiskReport(
        min_margin_level=min_ml,
        max_used_margin=float(used_margin.max()) if used_margin.size else 0.0,
        avg_used_margin=float(used_active.mean()) if used_active.size else 0.0,
        n_rejected_orders=int(n_rejected),
        n_stopouts=int(n_stopouts),
        blew_account=bool((equity <= 0.0).any()),
    )
